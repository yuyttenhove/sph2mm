...
pub fn sph_init_smoothing_lengths(particles: &mut [SphParticle], nngb: usize) {
    let rtree = RTree::bulk_load(
        particles
            .iter()
            .enumerate()
            .map(|(i, p)| LocId::new(p.loc.to_array(), i))
            .collect(),
    );
    for part in particles.iter_mut() {
        let query_loc = part.loc.to_array();
        let ngb = rtree
            .nearest_neighbor_iter(&query_loc)
            .take(nngb + 1)
            .last()
            .expect(&format!("There must be at least {:} particles!", nngb + 1));
...
impl<'a, K: SphKernel> SphInterpolator<'a, K> {
    pub fn init(particles: &'a [SphParticle], kernel: K) -> Self {
    ...
    pub fn interpolate(
        ...
        background_internal_energy: f64,
    ) -> Vec<SphParticle> {
        let rtree = RTree::bulk_load(
            centroids
                .iter()
                .enumerate()
                .map(|(i, c)| LocId::new(c.to_array(), i))
                .collect(),
        );
        ...
        for (p, w) in self
            ...
            .progress_with_style(
                ProgressStyle::with_template(
                    " -> Computing weights: {bar} {pos:>7}/{len:7} {elapsed_precise}",
                )
                .unwrap(),
            )
            .zip(sph_weights.iter_mut())
        {
        ...
        println!(
            " -> Total mass before redistributing: {:}",
            self.particles.iter().map(|p| p.mass).sum::<f64>()
        );
        ...
        for (p, &w_sum) in self
            ...
            .progress_with_style(
                ProgressStyle::with_template(
                    " -> Redistributing quantities: {bar} {pos:>7}/{len:7} {elapsed_precise}",
                )
                .unwrap(),
            )
            .zip(sph_weights.iter())
        {
            let h2 = p.smoothing_length * p.smoothing_length;
            let query_point = p.loc.to_array();
            for loc_id in rtree.nearest_neighbor_iter(&query_point) {
        ...
        let particles = conserved
            .iter()
            .zip(n_hit.iter().zip(generators.iter().zip(volumes.iter())))
            .map(|(cons, (&n, (x, vol)))| {
                ...
                if n > 0 {
                    ...
                    let part = SphParticle::init_background(
                        *x,
                        background_density,
        ...
        println!(
            " -> Number of background particles / Total: {:} / {:}",
            npart_bg,
            particles.len()
        );
        particles
